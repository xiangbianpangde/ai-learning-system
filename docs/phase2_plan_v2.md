...
**實現細節**:
- **IRT 模型**: 使用**二參數 Logistic 模型 (2PL)** — 移除猜測參數 c
  - 難度參數 (b): -3 到 +3
  - 區分度參數 (a): 0.5 到 2.0
  - ~~猜測參數 (c): 已移除~~
- **冷啟動策略**: 
...
class ItemResponse:
    ...
    # guessing: float  # c parameter — REMOVED in 2PL
...
class UserProgress:
    user_id: str
    current_level: int  # L1-L5
    questions_answered: int  # 用於冷啟動計數
    ability_estimate: float  # theta
    last_5_responses: List[bool]  # 冷啟動期記錄
```
...
> ⚠️ **注意**: 記憶保留率指標移至 Phase 3 驗收，Phase 2 僅實現基礎功能
...
class ForgettingCurve:
    ...
    retention_rate: float  # 0-1 (Phase 3 驗證)
```
...
- **覆蓋範圍**: **Phase 2 覆蓋單學科前 3 章（約 50-80 個知識點）**
  - 示例：高等數學前 3 章（函數與極限、導數與微分、微分中值定理）
  - 後續章節於 Phase 3 擴充
...
CREATE TABLE item_parameters (
    ...
    -- guessing DECIMAL(5,3),  -- c — REMOVED in 2PL
    difficulty_level INT,  -- L1-L5
    knowledge_point_id UUID,
    calibrated_by VARCHAR(50),  -- 'expert' | 'sample_test'
...
CREATE TABLE user_responses (
    ...
    is_cold_start BOOLEAN  -- 標記冷啟動期（前 5 題）
);
...
POST /api/v1/adaptive/next-item
  ...
  Response: { item_id, difficulty_level, estimated_success_rate, is_cold_start }

PUT /api/v1/adaptive/update-ability
  Body: { user_id, item_id, correct, response_time }
  Response: { new_ability, confidence_interval, questions_answered }

GET /api/v1/adaptive/user-progress/{user_id}
  Response: { current_level, ability_estimate, accuracy_rate, cold_start_complete }
```
...
GET /api/v1/path/recommend/{user_id}
  Response: { path: [{knowledge_point_id, chapter, order, estimated_time}], total_time, scope: "前 3 章" }
...
**目標**: 由教育專家預標定題目難度等級，作為 IRT 模型初始參數

**流程**:
1. **專家組成**: 3-5 名學科教育專家（如高等數學教師）
2. **標定標準**:
   - L1: 基礎概念記憶，無需計算
   - L2: 簡單公式應用，單步驟計算
   - L3: 標準綜合題，2-3 步驟
   - L4: 進階應用，多知識點結合
   - L5: 挑戰題，創新思維要求
3. **標定產出**: 每題專家評級 L1-L5 + 置信度評分
4. **參數轉換**: 根據評級映射初始 b 值（L1→b=-2, L3→b=0, L5→b=+2）
...
**目標**: 通過小樣本用戶測試校準題目參數，驗證專家評估準確性

**方案**:
1. **測試樣本**: 30 名用戶（覆蓋不同能力水平）
2. **測試題目**: 每知識點 3-5 題，共約 150-200 題
3. **數據收集**:
   - 作答正確率
   - 響應時間
   - 用戶主觀難度評分（1-5 分）
4. **參數校準**:
   - 使用最大似然估計 (MLE) 擬合 2PL 參數
   - 專家 b 值作為先驗，小樣本數據更新後驗
   - 區分度 a 根據正確率分佈計算
5. **驗收標準**: 專家評級與實測正確率相關係數 ≥ 0.7

**數據記錄**:
```python
class SampleTestCalibration:
    item_id: str
    sample_size: int  # 目標 30
    observed_accuracy: float
    avg_response_time: float
    calibrated_b: float
    calibrated_a: float
    expert_b_prior: float
...
| adaptive_difficulty | test_adaptive_difficulty.py | **2PL 參數估計**、難度調整邏輯、**冷啟動策略**、邊界條件 |
| forgetting_curve | test_forgetting_curve.py | 衰減係數計算、復習時間預測、SM-2 算法 |
| learning_path | test_learning_path.py | 依賴檢查、路徑規劃、跳過邏輯 |
| item_calibration | test_item_calibration.py | **專家評估轉換**、**小樣本參數擬合** |
...
- **指標**:
  ...
  - **難度調整響應時間 < 1 秒**
  - **用戶滿意度 ≥ 4/5（問卷調查）**
  - ~~遺忘曲線記憶保留率~~ → 移至 Phase 3
  - 學習效率提升 ≥ 20% (前後測對比)

---

## 七、時間表（修訂為 6 周開發 + 1 周測試）

| 階段 | 時間 | 任務 | 負責人 | 產出 |
|------|------|------|--------|------|
| **計劃** | Week 0 | 中書省起草、門下省審議 | 中書省 | 本計劃文檔 |
| **執行 W1-2** | Day 1-7 | 學習風格識別開發 | 工部 | learning_style.py + 測試 |
| **執行 W1-2** | Day 1-7 | 動態難度調整開發（2PL + 冷啟動） | 工部 | adaptive_difficulty.py + 測試 |
| **執行 W3-4** | Day 8-21 | 遺忘曲線校準開發 | 工部 | forgetting_curve.py + 測試 |
| **執行 W3-4** | Day 8-21 | 學習路徑自適應開發（前 3 章） | 工部 | learning_path.py + 測試 |
| **執行 W5-6** | Day 22-35 | **題目標定（專家 + 小樣本）** + API 集成 | 工部 + 測試組 | 標定數據 + 完整 API |
| **執行 W5-6** | Day 22-35 | 前端對接 + 數據庫遷移 | 工部 + 前端 | UI + 數據庫腳本 |
| **測試 W7** | Day 36-42 | 集成測試 + 用戶測試 + 性能優化 | 測試組 | 測試報告 + Bug 修復 |
| **上線** | Day 43-45 | 灰度發布 + 監控 | 运维 | 上線報告 |

**總工期**: 6 周開發 + 1 周測試 = 7 周（49 天）
...
| 風險 | 概率 | 影響 | 緩解措施 |
|------|------|------|----------|
| IRT 模型收斂慢 | 中 | 高 | **使用 2PL 降低複雜度**，預先專家標定題目參數 |
| 用戶行為數據不足 | 高 | 中 | **冷啟動策略**：新用戶默認 L3，前 5 題不調整 |
| 知識圖譜構建複雜 | 高 | 高 | **Phase 2 僅覆蓋前 3 章（50-80 點）**，迭代擴充 |
| 復習提醒推送延遲 | 中 | 中 | 使用消息隊列 + 重試機制 |
| 專家標定主觀偏差 | 中 | 中 | **小樣本測試校準**，30 人數據修正參數 |
...
| 學習風格 | 行為數據收集 | 完整 | 日誌檢查 |
| 學習風格 | 識別準確率 | ≥ 75% | 用戶測試 |
| 動態難度 | 題目難度標定 | 完整（2PL + 專家 + 小樣本） | 數據庫檢查 |
| 動態難度 | **追蹤延遲** | **< 1 秒** | 性能測試 |
| 動態難度 | **用戶滿意度** | **≥ 4/5** | **問卷調查** |
| 動態難度 | 冷啟動策略 | 新用戶 L3 + 前 5 題不調整 | 集成測試 |
| 遺忘曲線 | 基礎模型正確 | ✅ | 單元測試 |
| 遺忘曲線 | 個體校準誤差 | < 20% | 數據分析 |
| 遺忘曲線 | 復習提醒準時 | ✅ | 日誌檢查 |
| 遺忘曲線 | ~~記憶保留率~~ | **移至 Phase 3** | — |
| 學習路徑 | 知識依賴完整 | ✅（前 3 章） | 代碼審查 |
| 學習路徑 | 前置檢查準確 | ✅ | 集成測試 |
| 學習路徑 | 效率提升 | ≥ 20% | 用戶測試 |
...
| 修改項 | v1.0 | v2.0 | 門下省意見 |
|--------|------|------|-----------|
| IRT 模型 | 3PL（難度 b + 區分度 a + 猜測 c） | **2PL（難度 b + 區分度 a）+ 冷啟動策略** | 降低實現難度 |
| 知識圖譜範圍 | 未明確 | **單學科前 3 章（約 50-80 個知識點）** | 明確交付範圍 |
| 開發時間 | 4 周 | **6 周開發 + 1 周測試** | 時間充足 |
| 驗收標準 | 難度調整準確率≥80% | **響應時間<1 秒 + 用戶滿意度≥4/5** | 可測量 |
| 題目標定 | 缺失 | **專家評估 + 小樣本測試（30 人）** | 降低風險 |

---

**中書省起草完成（v2.0），提交門下省覆審。**
