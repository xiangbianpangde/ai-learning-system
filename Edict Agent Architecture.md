...
• 主题示例: task.created , task.planning , task.review.request , task.review.result ,
...
"payload": { ... },
"meta": { "priority": "normal", "model": "gpt-5-thinking", "version":
"1" }
}
...
"checkpoints": [ {"name":"fetch","status":"done"},
...
备注：此文档为可直接下载参考的架构设计，包含事件规范、WebSocket 协议、时序图和 JSON Schema，可
用于实现实时 agent 可观测系统。
5
